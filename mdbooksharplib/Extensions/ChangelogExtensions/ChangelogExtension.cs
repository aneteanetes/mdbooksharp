...
{
    public class ChangelogExtension : MdBookExtension<ChangelogExtensionConfig>
    {
        ...
        public override void BeforeRender(Book book)
        {
            if(this.Settings?.FullChangesUrl.IsEmpty() ?? false)
                return;
                
            var pageUrl = this.Settings.FullChangesUrl.Replace(".md", ".html");
            var changePage = new Page()
            {
                ...
                Path_Html = pageUrl,
                PathToRoot = Path.GetRelativePath(pageUrl.Replace(".md",".html"), "./").Replace("\\", "/").Substring(1),
...
";
        ...
        private HtmlNode ChangeLogNode(HtmlDocument doc, string innerHtml)
        {
            ...
            text += @$"<a href='{this.Settings.FullChangesUrl.Replace(".md", ".html")}' class='all-updates'>{this.Settings.FullchangeText}</a>
